;(() => {
  ...
  const ESTADOS_OCULTOS = new Set(["RETIRADO", "ENVIADO", "CANCELADO", "ENTREGADO"]);
  ...
  // =========================
  // Columnas esperadas:
  // ID | Cliente | DNI | Estado (texto) | Envío/Retiro | Último usuario | Acciones (botones)
      ...
      if (!acciones.length) {
        ...
        acciones.forEach((nuevoEstado) => {
          ...
          btn.addEventListener("click", async () => {
            ...
            const sucursalReal = String(p?.sucursal_retiro || "").toUpperCase().trim();
            if (!sucursalReal) {
              alert("Este pedido no tiene SUCURSAL_RETIRO. No se puede actualizar por seguridad.");
              return;
            }
